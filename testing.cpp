...
memcpy(node->mat, mat, sizeof node->mat); swap(node->mat[x][y], node->mat[newX][newY]); node->cost = INT_MAX;
node->level = level;
...
min->level + 1, min); child->cost = calculateCost(child->mat, final);
...
int x = 1, y = 2; solve(initial, x, y, final);
return 0;
}
